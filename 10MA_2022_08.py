def trade_with_ma(stock_no='0050.TW',start="2018-01-01",end= "2022-8-7",win=0.09,loss=0.03):
    # stock_no = '0050.TW'
    # start="2018-01-01"
    # end= "2022-8-7"
    # win=0.09
    ...
    count=0
    index = df2['trading_with_win/loss_ratio'].value_counts().index
    value = df2['trading_with_win/loss_ratio'].value_counts().values
    ...
    return f"\n原始資金為100\n單純用10日均線進出場,最後資金為{money:.2f},總交易次數為{len(total_sell)} \n用10日均線進場,出場用盈虧比,最後資金為{money2:.2f},總交易次數為{trad_time},勝率為{win_rate*100:.2f}%"
#######################################################################################
import pandas as pd
...
a=trade_with_ma("0050.TW","2015-01-01","2022-8-7",0.09,0.03)
print(a)